...
const char terminal_init[] PROGMEM =
  { "\x1B[0m\x1B[2J\x1B[0;0f" };
...
void
transmit_answer(unsigned char seqnum, unsigned int len)
{
  unsigned char cksum;
  unsigned char ch;
  int i;
...
void
programcmd(unsigned char seqnum)
{
...
unsigned char
terminalmode_readnum(unsigned char chr_nl)
{
  unsigned char ch;
...
void
terminalmode(unsigned char chr_nl)
{
  unsigned char i;
  // Init terminal
