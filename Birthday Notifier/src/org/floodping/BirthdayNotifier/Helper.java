...
public class Helper {
	...
	public static void Schedule(Context context)
	{
        ...
        if (dNow.after(dDate))
        {
        	lTime = dDate.getTime()  + (24 * 60 * 60 * 1000) ;
        }
        else
        {
        	lTime = dDate.getTime();
        ...
        Intent myIntent = new Intent("org.floodping.BirthdayNotifier.BirthdayCheckReceiver");
        PendingIntent pendingIntent = PendingIntent.getBroadcast(context, 0, myIntent, 0);
        
        am.cancel(pendingIntent);
        am.setRepeating(AlarmManager.RTC, lTime, 24 * 60 * 60 * 1000, pendingIntent);

        Toast.makeText(context, "Birthday Notifier scheduled in " + ((lTime - dNow.getTime())/60000) + " minutes", Toast.LENGTH_LONG).show();
	}
