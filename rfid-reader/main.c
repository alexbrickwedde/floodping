...
checkKey(uint8_t *d)
{
  if (d[0] == 0x00 && d[1] == 0x00 && d[2] == 0x00 && d[3] == 0x00 && d[4] == 0x00)
    return -1;
  uint8_t i;
...
main(void)
{
  ...
  while (1)
  {
    ...
    {
      if (bit_is_set(PIND, PD4) && bit_is_clear(PIND, PD5) && bit_is_set(PIND, PD6))
        saveKey(tag);

      if (bit_is_clear(PIND, PD4) && bit_is_set(PIND, PD5) && bit_is_set(PIND, PD6))
        deleteKey(tag);
