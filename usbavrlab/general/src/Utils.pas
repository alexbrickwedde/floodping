...
uses Classes,SysUtils
     {$IFDEF LCL}
     ,Graphics,Forms,Process,Dialogs,Clipbrd,FileUtil,Translations,
     ProcessUtils,LCLProc,TypInfo
     {$ENDIF}
     {$IFDEF MSWINDOWS}
     ,Registry,Windows
     {$ENDIF}
  ...
  If FindFirstUTF8(ProgramDirectory+'languages'+Directoryseparator+'*.'+lowercase(copy(lang,0,2))+'.po',faAnyFile,Info)=0 then
    repeat
      ...
      for i := 0 to po.Items.Count-1 do
        begin
          id := copy(TPoFileItem(po.Items[i]).Identifier,0,pos('.',TPoFileItem(po.Items[i]).Identifier)-1);
      ...
      for i := 0 to po.Items.Count-1 do
        begin
          ...
          for a := 0 to Screen.FormCount-1 do
            if UTF8UpperCase(Screen.Forms[a].ClassName) = id then
              begin
                id := copy(TPoFileItem(po.Items[i]).Identifier,pos('.',TPoFileItem(po.Items[i]).Identifier)+1,length(TPoFileItem(po.Items[i]).Identifier));
                        ...
                        if lowercase(copy(tmp,0,8)) = 'command=' then
                          begin
                            ...
                            if pos('%f',Result) = 0 then
                              ...
                              Stringreplace(Result,'%f','%s',[rfReplaceAll]);
//                            if not FileExists(copy(Result,0,pos(' ',Result)-1)) then
//                              begin
//                                apps := ExecProcessEx('whereis '+copy(Result,0,pos(' ',Result)-1));
...
function HTMLEncode(s: string): string;
begin
  Result := StringReplace(UTF8ToSys(s), '&', '&amp;', [rfreplaceall]);
  ...
  Result := StringReplace(result, 'Ö', '&Ouml;', [rfreplaceall]);
  Result := StringReplace(result, 'Ü', '&Uuml;', [rfreplaceall]);
  Result := SysToUTF8(StringReplace(result, 'ß', '&szlig;', [rfreplaceall]));
end;
...
begin
  ...
  Result := StringReplace(Result, '&Ouml;' ,'Ö', [rfreplaceall]);
  Result := StringReplace(Result, '&Uuml;' ,'Ü', [rfreplaceall]);
  Result := SysToUTF8(StringReplace(Result, '&szlig;','ß', [rfreplaceall]));
end;
