 ...
 *---------------------------------------------------------------------------------------------------------------------------------------------------
 */
uint8_t i2c_rtc_read(DATETIME * datetime, uint8_t bDST) {
	uint8_t rtcbuf[7];
	uint8_t rtc = 0;
	...
	*errorcode_p = i2c_master_init(); // init I2C interface
	...
	if (*errorcode_p == 0) {
		...
		if (i2c_write_ctrlreg()) {
			...
			i2c_rtc_sram_read(0x10, &rtc_dstactive, 1);
		} else {
...
void set_dst(uint8_t newstate) {
	if (newstate != rtc_dstactive) {
		rtc_dstactive = newstate;
		char s[100];
		i2c_rtc_sram_write(0x10, &rtc_dstactive, 1);
		sprintf(s, "dst now: %s\r\n", (newstate != 0) ? "active" : "inactive");
		uartPuts(s);
	}
}
